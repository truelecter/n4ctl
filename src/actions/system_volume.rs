...
mod win {
    ...
    impl EndpointVolume {
        ...
        /// Current master level 0.0..=1.0 (default render / console role).
        pub fn master_scalar(&self) -> Result<f32> {
            unsafe {
...
mod win {
    ...
    impl EndpointVolume {
        pub fn new() -> Self { Self }
        pub fn master_scalar(&self) -> Result<f32> {
            Err(anyhow!("system volume read is only implemented on Windows"))
        }
